	...
	<table width="100%" class="info">
		...
		{if $isValid.Editor.$journalId}
			<tr>
				...
				<td></td>
				<td align="center"><a href="{url journal=$journalPath page="editor" op="submissions" path="submissionsInReview"}">{translate key="article.articles"}</a>
				</td>
				<td align="left"><a href="{url journal=$journalPath page="editor" op="submissionsReport"}">{translate key="editor.reports.reportGenerator"}</a></td>
			</tr>
		{/if} 
		{if $isValid.SectionEditor.$journalId}
			...
			<tr>
				...
				<td align="center">{if $sectionEditorSubmissionsCount[0]}
					...
					{else}<span class="disabled">{translate key="common.queue.short.submissionsInReview"} (0)</span>{/if}
				</td>
		...
		{if $isValid.Reviewer.$journalId}
			{if (($user->isHsphMember() != true) && ($user->isCrecMember() != true))}
			{assign var="ERC" value="Technical Reviewer"} {/if}
			{assign var="reviewerSubmissionsCount" value=$submissionsCount.Reviewer.$journalId}
			<tr>
				<td colspan="2">&#187; <a href="{url journal=$journalPath page="reviewer"}">{if $ERC}{$ERC}{else}{translate key="user.role.reviewer"}{/if}</a></td>
				<td align="right">
				...
				{if !$user->isLocalizedTechnicalReviewer()}
				<td align="left">
