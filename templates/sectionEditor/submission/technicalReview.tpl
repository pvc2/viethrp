...
<div id="technicalReview">
	<h3>{translate key="common.technicalReview"}</h3>
	<table width="100%" class="data">
		<tr>
			<td title="{translate key="common.technicalReviewStatusInstruct"}" width="20%">[?] {translate key="common.status"}</td>
			<td width="80%">
				{if $lastDecisionArray.technicalReview == 1}
				{translate key="editor.article.decision.accept"}
				{else}
				{translate key="editor.article.decision.notRequired"}
				{/if}
			</td>
		</tr>
		{if $lastDecisionArray.technicalReview == 1}
