...
<table class="data" width="100%">
	<tr id="reviewersHeader" valign="middle">
		<td width="30%" colspan="2" ><h4>{translate key="common.technicalReview"}</h4></td>		
		<td width="35%" align="left" valign="middle">
			<a href="{url op="selectReviewer" path=$submission->getId()}" class="action">{translate key="editor.article.assignTechnicalReviewers"}</a>
		</td>
		<td width="35%" align="left" valign="middle">
			<form method="post" action="{url op="sendEmailNewMeeting"}")" enctype="multipart/form-data">
				<input type="hidden" name="articleId" value="{$submission->getId()}" />
				<input type="hidden" name="technicalReview" value="true"/>
...
<input type="hidden" name="technical" value="Yes"/>

{assign var="start" value="A"|ord}
{foreach from=$technicalReviewAssignments item=reviewAssignment key=reviewKey}
