...
<table width="100%" class="data">
	<tr>
        ...
        <tr>
		<td title="{translate key="common.idInstruct"}" width="20%" class="label">[?] {translate key="common.id"}</td>
	...
	<tr>
		...
		<td width="80%" class="value">
			{foreach name="suppFiles" from=$suppFiles item=suppFile}
            	<a href="{url op="downloadFile" path=$submission->getArticleId()|to_array:$suppFile->getFileId()}" class="file">{$suppFile->getFileName()|escape}</a>&nbsp;&nbsp;({$suppFile->getTypeTranslated()|escape})<br />
			{foreachelse}
				{translate key="common.none"}
			{/foreach}
		</td>
