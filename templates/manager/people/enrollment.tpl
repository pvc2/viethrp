...
<h3>{if $roleSymbolic=='reviewers'}ERC Members & Technical Reviewers{else}{translate key=$roleName}{/if}</h3>
<form method="post" action="{url path=$roleSymbolic}">
	<select name="roleSymbolic" class="selectMenu">
		...
		{/if}
		<option {if $roleSymbolic=='reviewers'}selected="selected" {/if}value="reviewers">ERC Members & Technical Reviewers</option>
...
{iterate from=users item=user}
	{if $user->isLocalizedTechnicalReviewer()!= "Yes"}
...
{iterate from=reviewers item=extreviewer}
	{if $extreviewer->isLocalizedTechnicalReviewer()== "Yes"}
	<tr valign="top">
