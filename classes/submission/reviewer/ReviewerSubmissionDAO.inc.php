...
class ReviewerSubmissionDAO extends DAO {
	...
	function &getReviewerSubmissionsByReviewerId($reviewerId, $journalId, $active = true, $searchField = null, $searchMatch = null, $search = null, $dateField = null, $dateFrom = null, $dateTo = null, 
		...
		// Modified by EL on August 18, 2012: Even if a reviewer didn t complete a review he can see it in archive		
		$params = array(
				'cleanScientificTitle', // Article title
				'cleanScientificTitle',
				$locale,
		...
		// Modified by EL on August 18, 2012: Even if a reviewer didn t complete a review he can see it in archive		
		if ($active) {
			$sql .=  ' AND r.date_completed IS NULL AND r.declined <> 1 AND (r.cancelled = 0 OR r.cancelled IS NULL) AND (ed.decision = '.SUBMISSION_EDITOR_DECISION_ASSIGNED.' OR ed.decision = '.SUBMISSION_EDITOR_DECISION_EXPEDITED.' OR (ed.decision = '.SUBMISSION_EDITOR_DECISION_COMPLETE.' AND r.review_type = "4"))';
		} else {
			$sql .= ' AND (r.date_completed IS NOT NULL OR r.cancelled = 1 OR r.declined = 1 OR ed.decision = '.SUBMISSION_EDITOR_DECISION_ACCEPT.' OR ed.decision = '.SUBMISSION_EDITOR_DECISION_RESUBMIT.' OR ed.decision = '.SUBMISSION_EDITOR_DECISION_DECLINE.' OR ed.decision = '.SUBMISSION_EDITOR_DECISION_DONE.' OR ((ed.decision ='.SUBMISSION_EDITOR_DECISION_ASSIGNED.' OR ed.decision = '.SUBMISSION_EDITOR_DECISION_EXPEDITED.' OR ed.decision = '.SUBMISSION_EDITOR_DECISION_EXEMPTED.') AND r.review_type = "4"))';
		}
	...
	/**
	 * Retrieve a submission for Full Review by article ID.
	 * @param $articleId int
	 * @param $reviewerId int
	 * @return ReviewerSubmission
	 */
...
/**
	...
	function getSubmissionsForERCReviewCount($reviewerId, $journalId) {
		...
		$sql = 'SELECT	r.date_completed, r.declined, r.cancelled, r.review_type, ed.decision
			FROM	articles a
				LEFT JOIN review_assignments r ON (a.article_id = r.submission_id)
				LEFT JOIN sections s ON (s.section_id = a.section_id)
				LEFT JOIN users u ON (r.reviewer_id = u.user_id)
		...
		while (!$result->EOF) {
			if ($result->fields['date_completed'] == null && $result->fields['declined'] != 1 && $result->fields['cancelled'] != 1 && (($result->fields['decision'] == 7 || $result->fields['decision'] == 8) || $result->fields['decision'] == 4 && $result->fields['review_type'] == 4)) {
				$submissionsCount[0] += 1;
			} else {
				$submissionsCount[1] += 1;
			}
		...
		if ($user->isHsphMember() && !$user->isCrecMember()) $sql .= ' AND s.section_id = 1';
		else if (!$user->isHsphMember() && $user->isCrecMember()) $sql .= ' AND s.section_id = 2';
